...
| Add dedicated transaction history route | Receipts are core Web3 UX and currently buried or missing. | `frontend/src/app/routes.tsx`, `frontend/src/layouts/AppLayout.tsx`, new `frontend/src/pages/TransactionHistoryPage.tsx`, `frontend/src/services/api/transactions.api.ts`, shared receipt components | `/app/transactions` lists confirmed transactions with filters, contract IDs, txHash, ledger, and Stellar Expert links. | Existing local mock transactions lack txHash; UI must label them unconfirmed or hide in live mode. | Open `/app/transactions`; filter by wallet, offer, loan, and type. Verify every live row has a working Stellar Expert link. |
...
6. Add Transaction History route.
7. Split Offer Detail from Loan Detail.
8. Rework Borrow Flow into accept -> activate with recovery.
9. Rework Create Offer stepper.
10. Add real balances and stronger oracle/admin UX.
